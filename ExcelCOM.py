...
class comobj(object):
    ...
    def __setattr__(self, name, value):
        if(name not in ["obj", "typ", "name", "_class", "n"]):
            self.typ.InvokeMember(name, BindingFlags.Instance |
                                  BindingFlags.SetProperty, None, self.obj, [value])
        else:
            super(comobj, self).__setattr__(name, value)
        ...
        def newm(*argsv):
            ...
            try:
                if(method):
                    ...
                    result = self.typ.InvokeMember(
                        name, BindingFlags.InvokeMethod | BindingFlags.GetProperty, None, self.obj, *argsv)
            except:
...
def GetExcel():
    ...
    if(not appExcel.Visible):
        appExcel.Visible = True
    if(not appExcel.Workbooks.Count):
